# Flow SOP 索引

> 15个核心场景的标准化操作流程

---

## 执行顺序

**每个场景SOP开始前，必须先完成3个公共SOP：**

```
用户需求 → 公共SOP 1 → 公共SOP 2 → 公共SOP 3 → 场景SOP
              ↓            ↓            ↓
          澄清需求     明确时间      获取数据
```

| # | 公共SOP | 文件 | 目的 |
|---|---------|------|------|
| 1 | 澄清需求 | [common/01-clarify-needs.md](common/01-clarify-needs.md) | 确认用户真实需求和必要信息 |
| 2 | 明确时间 | [common/02-define-time-window.md](common/02-define-time-window.md) | 确定分析日期、最近交易日、时段 |
| 3 | 获取数据 | [common/03-get-data.md](common/03-get-data.md) | 从可信来源获取数据 |

---

## 总索引

| 场景 | 文件 | 触发词 |
|------|------|--------|
| **赚钱引擎** | | |
| 选股 | [make-money/stock-picker.md](make-money/stock-picker.md) | 什么能赚钱/推荐股票/高PR选股 |
| 诊股 | [make-money/stock-diagnosis.md](make-money/stock-diagnosis.md) | 能买XXX吗/帮我看看 |
| 持仓诊断 | [make-money/position-diagnosis.md](make-money/position-diagnosis.md) | 要不要卖/持仓怎么办 |
| 解套 | [make-money/unstuck.md](make-money/unstuck.md) | 被套了/怎么解套 |
| 止损 | [make-money/stop-loss.md](make-money/stop-loss.md) | 亏多少走/止损设多少 |
| 止盈 | [make-money/take-profit.md](make-money/take-profit.md) | 赚多少跑/怎么分批卖 |
| **学习引擎** | | |
| 大盘前瞻 | [learn/market-outlook.md](learn/market-outlook.md) | 今天怎么看/大盘展望/制定交易计划 |
| 大盘总结 | [learn/market-summary.md](learn/market-summary.md) | 为什么涨/跌/帮我复盘 |
| 每日复盘 | [learn/daily-review.md](learn/daily-review.md) | 操作对吗/帮我复盘 |
| 案例拆解 | [learn/case-analysis.md](learn/case-analysis.md) | 为什么涨/为什么跌 |
| 错误反思 | [learn/error-reflection.md](learn/error-reflection.md) | 为什么亏/怎么避免 |
| 经验沉淀 | [learn/experience.md](learn/experience.md) | 怎么赚到的/成功经验 |
| **进化引擎** | | |
| 案例记录 | [evolve/case-record.md](evolve/case-record.md) | 记录这笔/归档 |
| 规律更新 | [evolve/rule-update.md](evolve/rule-update.md) | 更新规则/发现规律 |
| 模型优化 | [evolve/model-tune.md](evolve/model-tune.md) | 调整参数/优化模型 |

---

## 快速导航

### 赚钱引擎（买/卖/持）

```
公共SOP 1-2-3 → stock-picker.md
公共SOP 1-2-3 → stock-diagnosis.md
公共SOP 1-2-3 → position-diagnosis.md
公共SOP 1-2-3 → unstuck.md
公共SOP 1-2-3 → stop-loss.md
公共SOP 1-2-3 → take-profit.md
```

| 用户需求 | 跳转SOP |
|----------|---------|
| 不知道买什么 | [stock-picker.md](make-money/stock-picker.md) |
| 纠结买不买 | [stock-diagnosis.md](make-money/stock-diagnosis.md) |
| 纠结卖不卖 | [position-diagnosis.md](make-money/position-diagnosis.md) |
| 被套了怎么办 | [unstuck.md](make-money/unstuck.md) |
| 亏多少必须走 | [stop-loss.md](make-money/stop-loss.md) |
| 赚多少要跑 | [take-profit.md](make-money/take-profit.md) |

### 学习引擎（市场/反思）

| 用户需求 | 跳转SOP |
|----------|---------|
| 今天市场怎么看 | [market-outlook.md](learn/market-outlook.md) |
| 今天为什么涨/跌 | [market-summary.md](learn/market-summary.md) |
| 今天操作对吗 | [daily-review.md](learn/daily-review.md) |
| 这个股票为什么涨 | [case-analysis.md](learn/case-analysis.md) |
| 我为什么会亏 | [error-reflection.md](learn/error-reflection.md) |
| 我怎么赚到的 | [experience.md](learn/experience.md) |

### 进化引擎（积累/优化）

| 用户需求 | 跳转SOP |
|----------|---------|
| 记录这笔交易 | [case-record.md](evolve/case-record.md) |
| 发现新规律 | [rule-update.md](evolve/rule-update.md) |
| 调整参数 | [model-tune.md](evolve/model-tune.md) |

---

## SOP标准模板

每个SOP包含：

1. **公共SOP前置** — 必须先完成3个公共SOP
2. **触发词** — 用户可能问的典型问题
3. **前置确认** — 开始前需要收集的信息
4. **分析步骤** — 标准化的操作流程
5. **输出规范** — 报告的标准格式
6. **硬规则检查** — 必须遵守的规则
7. **关联SOP** — 相关场景指引
